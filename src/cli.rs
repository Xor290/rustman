...
pub enum OutputFormat { Markdown, Json }
...
pub fn print_usage() {
    ...
    eprintln!("  --payload-dir <dir>       Directory containing payload JSON files  [default: ./payload]");
...
pub async fn run(args: CliArgs) -> i32 {
    ...
    let spec_text = match std::fs::read_to_string(&args.openapi_file) {
        Ok(t)  => t,
        Err(e) => { eprintln!("error: cannot read {}: {e}", args.openapi_file); return 3; }
    };

    let parsed = match crate::openapi::parse(&spec_text) {
        Ok(p)  => p,
        Err(e) => { eprintln!("error: {e}"); return 3; }
    };
    ...
    let creds = {
        ...
        if let Some(b) = args.bearer        { c.bearer         = Some(b); }
        if let Some(k) = args.cookie        { c.cookie         = Some(k); }
        if let Some(h) = args.api_key_header { c.api_key_header = Some(h); }
        if let Some(v) = args.api_key_value  { c.api_key_value  = Some(v); }
        c
    };
    ...
    if payloads.is_empty() {
        eprintln!("error: no payload JSON files found in '{}'", args.payload_dir);
        return 3;
    }

    let total_payloads: usize = payloads.iter().map(|(_, p)| p.len()).sum();
    let total_requests: usize = endpoints.iter()
        .map(|ep| {
    ...
    while let Some(r) = rx.recv().await {
        if r.evidence.is_some() {
            ...
            eprintln!("[rustman] VULN {} {} param={} cat={}", ep.method, ep.path, r.param, r.category);
        }
        results.push(r);
    }
    ...
    eprintln!("[rustman] scan complete — {} requests, {} vulnerabilities", results.len(), vuln_count);
    ...
    if args.fail_on_vuln && vuln_count > 0 { 1 } else { 0 }
}
...
fn build_json(target: &str, endpoints: &[ApiEndpoint], results: &[ScanResult]) -> String {
    ...
    let vulns: Vec<serde_json::Value> = results.iter()
...
fn build_markdown(target: &str, endpoints: &[ApiEndpoint], results: &[ScanResult]) -> String {
    ...
    if by_cat.is_empty() {
        ...
        for (cat, rs) in &by_cat {
            let _ = writeln!(md, "| **{cat}** | {} | {} |", rs.len(), category_severity(cat));
        }
    }
    let _ = writeln!(md);
    ...
    if !by_cat.is_empty() {
        ...
        for (cat, rs) in &by_cat {
            for r in rs {
                ...
                let _ = writeln!(md, "### Finding #{idx} — {cat} ({})", category_severity(cat));
                let _ = writeln!(md);
                let _ = writeln!(md, "| Field | Value |");
                let _ = writeln!(md, "|---|---|");
    ...
    for (i, ep) in endpoints.iter().enumerate() {
        let params: Vec<String> = ep.body_fields.iter()
            ...
            .collect();
        let params_s = if params.is_empty() { "—".into() } else { params.join(", ") };
        let ep_vulns: Vec<String> = results.iter()
            .filter(|r| r.ep_idx == i && r.evidence.is_some())
            .map(|r| format!("{} ({})", r.category, r.param))
            .collect();
        let vuln_s = if ep_vulns.is_empty() { "—".into() } else { ep_vulns.join(", ") };
        let _ = writeln!(md, "| **{}** | `{}` | {} | {} |", ep.method, ep.path, params_s, vuln_s);
    }
