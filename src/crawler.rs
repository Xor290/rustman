...
pub enum CrawlMsg {
    Visiting { url: String, depth: usize, request: Vec<u8> },
    Done     { url: String, status: u16, new_links: usize, response: Vec<u8> },
...
pub fn parse_url(url: &str) -> Option<UrlParts> {
    parse_url_parts(url).map(|(host, port, tls, path)| UrlParts { host, port, tls, path })
}
...
pub async fn run(
    ...
    while let Some((url, depth)) = queue.pop_front() {
        ...
        let (host, port, tls, path) = match parse_url_parts(&url) {
            Some(p) => p,
            None => {
                let _ = tx.send(CrawlMsg::Failed { url, reason: "Unparseable URL".into() });
                continue;
            }
        };
        ...
        let _ = tx.send(CrawlMsg::Visiting {
            url: url.clone(), depth, request: request_bytes.clone(),
        });
        ...
        let _ = tx.send(CrawlMsg::Done { url, status, new_links, response: resp });
    }
...
fn split_response(resp: &[u8]) -> (u16, String) {
    let status = std::str::from_utf8(resp).unwrap_or("")
        .lines().next()
        .and_then(|l| l.split_whitespace().nth(1))
        .and_then(|s| s.parse().ok())
        .unwrap_or(0);
    let body = resp.windows(4)
        .position(|w| w == b"\r\n\r\n")
...
fn is_crawlable(url: &str) -> bool {
    ...
    const SKIP: &[&str] = &[
        ...
        ".pdf", ".zip", ".tar", ".gz", ".mp4", ".mp3", ".ogg", ".wav",
        ".woff", ".woff2", ".ttf", ".eot",
        ".css", ".js", ".json", ".xml", ".txt", ".csv", ".map",
    ];
...
fn extract_links(html: &str, base_host: &str, base_tls: bool, base_port: u16, current_path: &str) -> Vec<String> {
    let lower = html.to_ascii_lowercase();
    let mut pos = 0;
    let mut links = Vec::new();

    while pos < lower.len() {
        let Some(rel) = lower[pos..].find("href=") else { break };
        pos += rel + 5;
        ...
        let Some(end_rel) = lower[start..].find(q) else { break };
...
fn resolve(href: &str, base_host: &str, base_tls: bool, base_port: u16, current_path: &str) -> Option<String> {
    let href = href.trim();
    if href.is_empty()
        || href.starts_with('#')
    ...
    if href.starts_with("http://") || href.starts_with("https://") {
        ...
        if host.eq_ignore_ascii_case(base_host) { Some(href.to_string()) } else { None }
    } else if href.starts_with("//") {
        let host = href[2..].split('/').next().unwrap_or("").split(':').next().unwrap_or("");
        ...
        let dir = current_path.rfind('/').map(|i| &current_path[..=i]).unwrap_or("/");
